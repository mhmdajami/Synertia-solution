...
export default function App() {
  const [language, setLanguage] = useState(() => {
    ...
    try {
      return localStorage.getItem("synertia-language") === "fr" ? "fr" : "en";
    } catch {
      return "en";
    }
  });
  ...
  useEffect(() => {
    ...
    document.title = language === "fr"
      ? "Synertia Solutions | Ingénierie industrielle & Automatisation"
      : "Synertia Solutions | Industrial Engineering & Automation";
    try {
      ...
      <header className="fixed inset-x-0 top-0 z-50 border-b border-white/10 bg-ink/90 text-white backdrop-blur-xl">
        <div className="mx-auto flex h-[76px] max-w-7xl items-center justify-between px-5 lg:px-8">
          ...
          <div className="flex items-center gap-2">
            <div className="flex h-10 items-center border border-white/20 p-1" aria-label={t.language}>
              {["en", "fr"].map((code) => (
                <button
                  key={code}
                  type="button"
                  onClick={() => changeLanguage(code)}
                  className={`h-8 px-2.5 text-[11px] font-extrabold uppercase tracking-wider transition-colors ${
                    language === code ? "bg-white text-ink" : "text-slate-400 hover:text-white"
                  }`}
